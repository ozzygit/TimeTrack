...
# Default to published exe if not specified
if ([string]::IsNullOrEmpty($ExePath)) {
    $ExePath = Join-Path $projectRoot "bin\Release\net8.0-windows\win-x64\publish\TimeTrackv2.exe"
}
...
function Test-ExecutionLocation {
  ...
  New-Item -ItemType Directory -Path $Location -Force | Out-Null
    ...
    # Copy exe
    $testExe = "$Location\TimeTrackv2.exe"
...
Test-ExecutionLocation "C:\Apps\TimeTrack" "C:\Apps\TimeTrack (Recommended)"
